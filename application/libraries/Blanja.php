...
class Blanja
{
    ...
    const API_BLANJA_APPID = self::API_BLANJA_APPID_PROD;
    const API_BLANJA_APPKEY = self::API_BLANJA_APPKEY_PROD;
    ...
    public function orderDetails($inputs) {
        ...
        if (!empty($commonParams['access_token'])) {
            $reqParams['sign'] = $this->_generatesign($reqParams);
            Log4php::getLogger()->debug("orderDetails Params : ".json_encode($reqParams));
            $res = $this->curl_get($path.'?'.http_build_query($reqParams));
            return $res;
        } else {
            return array();
